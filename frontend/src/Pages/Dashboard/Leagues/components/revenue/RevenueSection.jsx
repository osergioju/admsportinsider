  ...
  const availableYears = useMemo(() => {
    ...
    Object.values(data || {}).forEach((clubData) => {
      clubData.forEach((item) => {
        if ((item.code === "recurring_revenue" || item.code === "revenue") && Number(item.converted_value) !== 0) {
          years.add(item.year);
        }
      });
    });
    ...
    <div className="w-full bg-white lg:p-10 p-4 md:p-6 rounded-xl">
      {mainData !== undefined && !mainData.length ? (
        ...
        <>
          ...
          <span className="text-xs lg:text-sm xl:text-base opacity-30 inline-block mb-1 -translate-y-1">{t("club.finance.in_millions", "em milhões")}</span>
          <ChartFilter
