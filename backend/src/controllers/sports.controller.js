  ...
  try {
    const compRes = await db.query(`
      SELECT cs.id_competition_season, l.id_league, l.name AS league_name,
             l.logo_url, cs.id_season, s.year
      FROM club_competition_stats ccs
      JOIN competition_seasons cs ON cs.id_competition_season = ccs.id_competition_season
  ...
  try {
    const baseQuery = `
      ...
      WHERE
        ...
        AND ($2 = '' OR position_pt = $2)
        AND ($3 = '' OR nationality ILIKE '%' || $3 || '%')
    `;
...
export async function getPlayerDetail(req, res) {
  ...
  try {
    const playerRes = await db.query(`
      SELECT p.id_player, p.full_name, p.birthday, p.position,
...
export async function getLeagueSports(req, res) {
  ...
  try {
    ...
    const standings = {
      ...
      home:  [...standingsRes.rows].sort((a,b)=>(a.position_home??999)-(b.position_home??999)).map(r => mkRow(r,'position_home','wins_home','draws_home','losses_home','goals_for_home','goals_against_home','matches_home')),
      away:  [...standingsRes.rows].sort((a,b)=>(a.position_away??999)-(b.position_away??999)).map(r => mkRow(r,'position_away','wins_away','draws_away','losses_away','goals_for_away','goals_against_away','matches_away')),
    };
    ...
    res.json({ league, seasons, season, standings, matches });
  } catch (err) {
