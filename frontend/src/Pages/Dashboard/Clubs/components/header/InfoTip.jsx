...
export default function InfoTip({ title, children, label = "O que é isto?", placement = "top", color = "currentColor" }) {
    const id = useId();
        ...
        >
            ...
            {open && (
                <span
                    role="tooltip"
                    id={id}
                    style={{ position: "absolute", zIndex: 30, ...(placement === "bottom-end" ? { top: "calc(100% + 10px)", right: -8 } : { bottom: "calc(100% + 10px)", left: -8 }), width: 280, maxWidth: "min(280px, 70vw)", padding: "12px 14px", borderRadius: 14, background: "var(--dm-surface-2, #fff)", border: "1px solid var(--dm-border, #ECE9F6)", boxShadow: "0 18px 40px -16px rgba(5,1,17,.35)", color: "var(--dm-text, #050111)", textAlign: "left", textTransform: "none", letterSpacing: "normal", whiteSpace: "normal", fontSize: 13, fontWeight: 400, lineHeight: 1.45 }}
                >
