...
import {
  ...
  AlertTriangle, ChevronRight, XCircle, EyeOff, Trash2,
} from "lucide-react";
...
export default function UploadMatchesPage() {
  ...
  const [hiddenClubs, setHiddenClubs]         = useState({}); // { "csvTeamName": true } — created as hidden
  const [creatingHidden, setCreatingHidden]   = useState({}); // { "csvTeamName": true } — loading state
  ...
  useEffect(() => {
    api.get("/admin/leagues?limit=500")
      .then(({ data }) => setAllLeaguesList(data.leagues ?? []))
      .catch(() => {});
  }, []);

  async function handleDeleteMatches() {
    if (!deleteLeague || !deleteYear) return alert("Selecione a liga e informe o ano.");
    if (!window.confirm(`Apagar todas as partidas da liga selecionada na temporada ${deleteYear}?`)) return;
    try {
      setDeleting(true);
      setDeleteResult(null);
      const { data } = await api.delete(`/upload/import/matches/${deleteLeague}/seasons/${deleteYear}`);
      setDeleteResult(data);
    } catch (err) {
      alert(err?.response?.data?.error || "Erro ao apagar partidas.");
    } finally {
      setDeleting(false);
    }
  }
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      setPreview(data);
      setSeasonOverride(String(data.detectedYear ?? ""));
      const init = {};
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 sm:p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white rounded-3xl border border-gray-200 shadow-xl p-8 space-y-6 animate-in zoom-in-95 duration-300">
            ...
            <div className="flex gap-3">
              <div className="flex-1 p-3 bg-purple-50 rounded-2xl border border-purple-100">
                <p className="text-[10px] uppercase font-black text-purple-400 mb-1">
                  Ano da temporada
                  {seasonOverride && String(seasonOverride) !== String(preview.detectedYear) && (
                    <span className="ml-1 normal-case font-normal text-amber-500">(detectado: {preview.detectedYear})</span>
                  )}
                </p>
                <input
                  type="number"
                  ...
                  className="w-full bg-transparent text-sm font-bold text-purple-700 focus:outline-none focus:ring-1 focus:ring-[#7F33D9] rounded px-1 -mx-1"
                  placeholder={String(preview.detectedYear ?? "")}
                />
              </div>
              <div className="flex-1 p-3 bg-purple-50 rounded-2xl border border-purple-100">
        ...
        {step === "done" && importResult && (
          <div className="border-t border-gray-100 pt-6 space-y-4 animate-in fade-in">
            ...
            <div className="flex justify-center">
              <button onClick={() => { setStep("upload"); setFile(null); setPreview(null); setImportResult(null); setLeague(""); setSelectedCountry(""); setClubMappings({}); setHiddenClubs({}); setCreatingHidden({}); }} className="text-sm text-[#7F33D9] font-bold hover:underline">
                Fazer novo upload
              </button>
            </div>
          </div>
      ...
      {/* Apagar partidas de uma temporada */}
      <div className="bg-white rounded-3xl border border-red-100 shadow-xl p-8 space-y-5 mt-6 animate-in fade-in">
        <div className="flex items-center gap-2">
          <Trash2 size={16} className="text-red-400" />
          <p className="text-sm font-bold text-red-500 uppercase tracking-widest">Apagar Partidas</p>
        </div>
        <p className="text-xs text-gray-400">Remove todas as partidas (e suas estatísticas) de uma liga em uma temporada específica. Ação irreversível.</p>
        <div className="grid grid-cols-2 gap-3">
          <div>
        ...
        {deleteResult && (
          <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-100 rounded-2xl text-sm text-green-700 font-semibold">
            <CheckCircle2 size={14} /> {deleteResult.deleted} partida(s) apagada(s).
          </div>
        )}
        <div className="flex justify-center">
          <button
          ...
          >
            {deleting ? <><Loader2 className="animate-spin w-4 h-4" /> Apagando...</> : <><Trash2 size={16} /> Apagar Partidas</>}
          </button>
        </div>
      </div>
    </div>
